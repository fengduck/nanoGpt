...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.c_attn = nn.Linear(config.n_embd, 3 * config.n_embd, bias=config.bias)  #Q,K,V
        # output projection
        self.c_proj = nn.Linear(config.n_embd, config.n_embd, bias=config.bias)
        # regularization
        self.attn_dropout = nn.Dropout(config.dropout)  #去除一定百分比的神经元
        self.resid_dropout = nn.Dropout(config.dropout)
        self.n_head = config.n_head
        self.n_embd = config.n_embd
        self.dropout = config.dropout
        # flash attention make GPU go brrrrr but support is only in PyTorch >= 2.0
        # 判断你的PyTorch版本里有没有内置的高性能注意力函数scaled_dot_product_attention 返回布尔类型
    ...
    def forward(self, x):
        ...
        q, k, v  = self.c_attn(x).split(self.n_embd, dim=2)  # 对最后一个维度划分
        ...
        if self.flash:
            ...
            y = torch.nn.functional.scaled_dot_product_attention(q, k, v,
                                                                 attn_mask=None, # 不额外加 mask（因为 causal 直接开启了）
                                                                 dropout_p=self.dropout if self.training else 0,
                                                                 is_causal=True # 设置为“因果注意力”
                                                                 # 关键点！表示这是 GPT 等模型中使用的 “不能看未来”的注意力机制。
                                                                 # is_causal=True 会自动帮你加上这个 “三角形” 的 mask：
                                                                 )
        else:
